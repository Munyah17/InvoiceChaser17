...
export default function CreditNotePage() {
  ...
  const [noteNum, setNoteNum] = useState(`CN-${Date.now().toString().slice(-6)}`)
  ...
  const exportPDF = () => {
    ...
    doc.setFontSize(12)
    doc.text('This credit note can be applied against future invoices.', 14, 95)
    doc.save(`CreditNote_${noteNum}.pdf`)
  }
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 rounded-xl p-5 space-y-4 max-w-lg">
        ...
        <div>
          ...
          <input className="w-full mt-1 px-3 py-2 text-xs border border-neutral-200 dark:border-neutral-700 rounded-lg bg-neutral-50 dark:bg-neutral-950 text-neutral-900 dark:text-white outline-none" value={reason} onChange={e => setReason(e.target.value)} placeholder="e.g., Overpayment, Goods returned, Discount applied" />
        </div>
        <div>
        ...
        <div className="border-t border-neutral-100 dark:border-neutral-800 pt-3 flex justify-end">
          <div className="text-right">
            <div className="text-[10px] text-neutral-500 dark:text-neutral-400">Total Credit</div>
